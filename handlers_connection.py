"""Connection lifecycle for Jobber Connector."""
...
@chat.function(
    "connect_jobber",
    "Connect Jobber account via credentials.",
    action_type="write",
    chain_callable=True,
    event="jobber-connector.connect_jobber",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_jobber(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Jobber Connector."""
    client = JobberClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Jobber Account",
...
@chat.function(
    "list_connections",
    "List connected Jobber accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_jobber",
    "Disconnect Jobber account.",
    action_type="write",
    chain_callable=True,
    event="jobber-connector.disconnect_jobber",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_jobber(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
